...
export async function controlApp(config: LoadedConfig, action: AppAction, options: AppOptions = {}, dependencies: Dependencies = {}) {
  ...
  if (config.app.type === 'expo' && config.app.launchTarget === 'expo-go') throw new CliError('WORKFLOW_UNSUPPORTED', 'Expo Go app control is not implemented yet');
...
async function resolveExpoProjectUrl(config: LoadedConfig, secrets: string[], dependencies: Dependencies): Promise<string> {
  ...
  const url = await (dependencies.resolveExpoUrl ?? (async (port) => {
    let response = await fetch(`http://127.0.0.1:${port}/_expo/open?platform=ios&runtime=custom`, { redirect: 'manual' });
    if (response.status === 404) response = await fetch(`http://127.0.0.1:${port}/_expo/link?platform=ios&choice=expo-dev-client`, { redirect: 'manual' });
  ...
  if (!parsed.protocol.startsWith('exp+') || parsed.hostname !== 'expo-development-client' ||

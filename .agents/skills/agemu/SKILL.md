...
1. Inspect `.agemu.json`. If absent, identify the app type and an available Simulator with `agemu simulator list`. Use `agemu setup --udid=ID`, or add `--expo-go` for Expo Go. Noninteractive setup selects the sole booted Simulator when `--udid` is absent. If setup cannot run, write a version 2 config using the [config examples](../../../README.md#configure-an-app). Set the selected Simulator UDID. Ask only for choices the repository cannot settle.
...
4. Verify the feature with `agemu observe` and a bounded `agemu ui run` plan. Prefer accessibility identifiers, then exact labels, then coordinates. Set swipe `duration` when gesture speed matters. Follow animations with `{ "wait": { "duration": 0.5 } }` before a screenshot, and follow taps with a wait or assertion for the expected screen. Use `--backend=xctest` for an `.xcresult`, or let the CLI select `idb` when suitable. Inspect the returned backend and artifacts.
